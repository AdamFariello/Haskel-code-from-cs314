...
--     THIS REPLACES THE FUM FUNCTION IN APPLICATION
--     IE:
--     sum [2,3] -> foldr (+) 0 [2,3]
--     sum :: (Num a) => [a] -> a
--     sum = foldr (+) 0
...
--f := take three things, input has to be same value of return
...
minTree = foldTree f z
    where
    f = (\a b c -> minimum(filter (Nothing /=) ([a] ++ [Just b] ++ [c])))
    z = Nothing
...
foldRose f (Node v []) = f v []
foldRose f (Node v x)  = f v (map (foldRose f) (x))
...
{-
    [ Node 7 [], Node 4 [Node 2 [], Node 100 []], Node 8 [], Node 9 [Node 9 []]]

    [ Node 7 [], Node 4                  , Node 8 [], Node 9     ]
                 [Node 2 [], Node 100 []]             [Node 9 []]
-}
-- we should have:
--   sumRose rose1 == 157
...
minRose = (\x -> minimum(foldRose f x))
    where
    f = (\a b -> ([a] ++ (foldl (++) [] b)))
